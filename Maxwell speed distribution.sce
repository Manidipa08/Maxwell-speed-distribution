...
show_window(1)
plot(v,f(v),'k','thickness',2)
plot(avg*ones(1,N),d,'-*-g')
plot(rms*ones(1,N),d,'-')
plot(v(mp)*ones(1,N),d,'--m','thickness',2)
...
for i = 1:6
function y=f(v)
y = ((4*%pi)*(m/(2*%pi*kb*temp(i)))^(3/2)*v^2).*exp((-
m*v^2)/(2*kb*temp(i)))
endfunction
func = f(v)
plot2d(v,func,i)
...
legend("300K", "500K", "1200K", "1500K", "2000K", "2500K")
xlabel("Velocity (m/s)")
ylabel('Function (v)')
xgrid()
end
...
for j = 1:6
function y=f(velo)
y = ((4*%pi)*(m_atomic(j)/(2*%pi*kb*T))^(3/2)*velo^2).*exp((-
m_atomic(j)*velo^2)/(2*kb*T))
endfunction
func_m = f(velo)
plot2d(velo,func_m,j)
a=gca();
a.children
poly1= a.children(1).children(1);
poly1.thickness = 2;
...
title('Maxwell Velocity Distribution Function (T = 600 K)')
legend('Function (v)','RMS','Most Probable','Average')
xlabel("Velocity (m/s)")
ylabel('Function (v)')
xgrid()
...
func_T = f(v_dd)
plot2d(v_dd,func_T,ii)
a=gca();
a.children
poly1= a.children(1).children(1);
poly1.thickness = 2;
title('Maxwell Velocity Distribution Function ')
...
for jj = 1:6
function y=f(v_dd)
y = ((m_atomic(jj)/(2*%pi*kb*T))^(1/2).*exp((-
m_atomic(jj)*v_dd^2)/(2*kb*T)))
endfunction
func_g = f(v_dd)
plot2d(v_dd,func_g,jj)
a=gca();
a.children
poly1= a.children(1).children(1);
poly1.thickness = 2;
